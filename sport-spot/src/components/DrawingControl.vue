<template>
  <section>
    ...
    <button :disabled="!hasActiveLayers" type="button" @click="clear">Clear</button>
     <button :disabled="!hasActiveLayers" type="button" @click="getDrawing">Get Data</button>
  </section>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      clear() {
        ...
        //console.log(JSON.stringify(this.vector.getSource().getFeature(0).getCoordinates()));
       let source = this.vector.getSource();
       source.forEachFeature(function(feature) {
          let coord = feature.getGeometry().getCoordinates();
          console.log(coord);
          let coordT = toLonLat(coord, 'EPSG:3857'  )
         console.log(coordT);
        });
      }
    }
