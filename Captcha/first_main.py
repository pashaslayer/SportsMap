...
def create_captcha_svg(captcha_text):
    ...
    for i in range(len(captcha_text)):
        ...
        dwg.add(dwg.text(char, insert=(x, y), fill='red', font_size=30, transform=f"rotate({rotation}, {x}, {y})"))
        ...
        for _ in range(10):
            ...
            dwg.add(dwg.line(start=(x1, y1), end=(x2, y2), stroke=svgwrite.rgb(300, 100, 0, '%')))
